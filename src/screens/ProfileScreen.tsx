...
const patchDoctorProfile = async (
  ...
  const { data } = await api.patch(
    `/api/doctors/edit-profile/${doctorId}`,
    body,
  ); // ✅ New route
            ...
            return (
              ...
              >
                <Text
                  style={[
                    cs.itemTxt,
                    sel && { color: C.primary, fontWeight: '800' },
                  ]}
                >
                  {item}
                </Text>
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          <Text
                            style={[
                              s.certStatus,
                              {
                                color: cert.isExisting ? C.success : C.warning,
                              },
                            ]}
                          >
                            {cert.isExisting
...
const cs = StyleSheet.create({
  ...
  sheet: {
    ...
    borderTopRightRadius: scale(28),
    maxHeight: '72%',
    paddingBottom: Platform.OS === 'ios' ? scale(24) : 0,
  },
  handle: {
    width: scale(40),
    height: scale(5),
          ...
          >
            ...
            <View style={[exm.fieldWrap, { zIndex: 20 }]}>
              ...
              >
                <Text
                  style={{
                    fontSize: scale(14),
                    color: designationDropdown ? C.ink : C.textMuted,
                    flex: 1,
                    fontWeight: '600',
                  }}
                >
                ...
                <Ionicons
                  name={showDesigList ? 'chevron-up' : 'chevron-down'}
                  size={scale(16)}
                  color={C.textMuted}
                />
              </TouchableOpacity>
            ...
            <View style={exm.fieldWrap}>
              ...
              >
                <Text
                  style={{
                    color: startDate ? C.ink : C.textMuted,
                    fontSize: scale(14),
                    fontWeight: '600',
                  }}
                >
            ...
            {!isCurrent && (
              <View style={exm.fieldWrap}>
                ...
                >
                  <Text
                    style={{
                      color: endDate ? C.ink : C.textMuted,
                      fontSize: scale(14),
                      fontWeight: '600',
                    }}
                  >
...
const ProfileScreen: React.FC = () => {
    ...
    async (isRefresh = false) => {
      ...
      try {
        ...
        Alert.alert('Error', err.message || 'Failed to load profile.', [
          { text: 'Retry', onPress: () => fetchProfile(false) },
        ]);
      } finally {
  ...
  const updateProfile = async (
    ...
    setSaving(true);
    try {
      // Note: ensure this points to `/edit-profile/${doctorId}` if you applied the previous fix
    ...
    confirmDelete(
      ...
      async () => {
        ...
        try {
          ...
          const { data } = await api.patch('/api/doctors/update-profile', {
    ...
    <SafeAreaView style={styles.root} edges={['top', 'bottom']}>
      ...
      <Animated.ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={onRefresh}
            colors={[C.primary]} // For Android
            tintColor={C.primary} // For iOS
          />
        }
      >
        ...
        <SectionCard
          ...
          icon="👤"
          onEdit={() => navigation.navigate('EditDoctorProfileScreen')}
        >
          <View style={styles.twoCol}>
            <View style={styles.col}>
              ...
              <InfoRow
                label="Date of Birth"
                value={formatDateString(profile.date_of_birth)}
              />
        ...
        {/* ── EXPERIENCE SECTION ── */}
        <SectionCard
          title="Experience & Clinical Areas"
          icon="🏥"
                  ...
                  >
                    ...
                    {activeAreas.length > 0 && (
                      ...
                      >
                        <Ionicons
                          name={
                            showClinicalAreas ? 'chevron-up' : 'chevron-down'
                          }
                          size={scale(16)}
                          color={C.primary}
                        />
            ...
            profile.experience.map((exp, i) => (
              ...
              >
                ...
                <View style={styles.expBody}>
                  ...
                  <Text style={styles.expDate}>
                    {exp.start_date
                      ? `${formatDateString(exp.start_date as string)} → `
                      : ''}
                    {exp.is_current
                      ? 'Present'
                      : exp.end_date
                      ? formatDateString(exp.end_date as string)
                      : ''}
                  </Text>
                  <Text style={styles.expMeta}>
              ...
              {profile.references.map((ref, i) => (
                <View key={(ref as any)._id || i} style={styles.refCard}>
                  ...
                  {(ref as any).ref_clinic && (
                    <Text
                      style={[styles.refProfession, { marginTop: scale(2) }]}
                    >
                      🏥 {(ref as any).ref_clinic}
                    </Text>
                  )}
          ...
          <View style={styles.docRow}>
            ...
            <View style={styles.docBody}>
              ...
              {profile.resume_url ? (
                <TouchableOpacity
                  onPress={() =>
                    profile.resume_url && Linking.openURL(profile.resume_url)
                  }
                  activeOpacity={0.7}
                >
            ...
            <View style={{ flexDirection: 'row', gap: scale(6) }}>
              ...
              {!!profile.resume_url && (
                ...
                >
                  {deletingResume ? (
                    <ActivityIndicator color={C.urgent} size="small" />
                  ) : (
                    <Ionicons
                      name="trash-outline"
                      size={scale(14)}
      ...
      <EducationModal
        ...
        onSave={updatedEducation => {
          const isEdit = eduModalEditIndex !== undefined;
          const message = isEdit
            ? 'Education updated successfully'
            : 'Education added successfully';

          // Pass the message to the updateProfile function
          updateProfile({ education: updatedEducation }, message);
        ...
        >
          <View style={prefStyles.modalHeader}>
            <TouchableOpacity
              ...
              style={prefStyles.modalCloseBtn}
            >
              <Ionicons name="chevron-back" size={scale(22)} color={C.ink} />
            </TouchableOpacity>
...
topHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'flex-start', // ⬅️ Changed from 'space-between'
    gap: scale(16),               // ⬅️ Added a gap for spacing
    backgroundColor: C.cardBg,
    paddingHorizontal: scale(20),
    paddingVertical: scale(14),
    borderBottomWidth: 1,
